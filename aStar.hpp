...
public:
    ...
    bool find_path(double x1, double y1, double x2, double y2, vector<vector<double>>& res) {
        int start_x = (int) (x1 * 4 / 4);
        int start_y = (int) (y1 * 4 / 4);
        int end_x = (int) (x2 * 4 / 4);
        int end_y = (int) (y2 * 4 / 4);
        vector<Node *> path;
...
//        cout << "Time taken: " << duration.count() << " microseconds" << endl;
        ...
        for (int i = path.size() - 1; i >= 0; --i) {
            double res_x = path[i]->x/1.0 ;
            double res_y = path[i]->y/1.0 ;
            res.push_back({res_x+0.5, res_y+0.5});
            fprintf(stderr, "(%lf, %lf)", res_x-0.5, res_y+0.5);
